...
class SwinBlock(nn.Module):
    """
    轻量级 Swin Transformer Block（窗口注意力 + MLP），用于替换瓶颈的普通 Transformer。
    - 输入/输出形状均为 [B, N, C]，其中 N = H*W。
    - 通过传入当前特征图尺寸 (H, W) 与窗口大小 window_size 实现窗口划分。
    注意：为保持脚本其它逻辑不变，这里使用简化版窗口注意力，不包含移位（shifted window）。
    """
    def __init__(self, dim, heads, window_size=8, mlp_ratio=4.0):
        super().__init__()
        self.dim = dim
        self.heads = heads
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        assert N == H * W, "Token length must equal H*W"
        ws = self.window_size
        # 仅支持当 H,W 能整除窗口大小的情况（当前网络使用固定 1/8 尺度，满足条件）
        assert H % ws == 0 and W % ws == 0, "H and W must be divisible by window_size"
        # [B, H, W, C]
        x_img = x.view(B, H, W, C)
        # 按窗口划分 -> [B, nH, ws, nW, ws, C]
        nH = H // ws
        nW = W // ws
        x_win = x_img.view(B, nH, ws, nW, ws, C)
        # 重排每个窗口为序列 -> [B, nH*nW, ws*ws, C]
        x_win = x_win.permute(0, 1, 3, 2, 4, 5).contiguous().view(B, nH * nW, ws * ws, C)
        # 在每个窗口内做多头注意力
        x_norm = self.norm1(x_win)
        # MultiheadAttention 期望 [B*windows, L, C]
        x_att_in = x_norm.view(B * nH * nW, ws * ws, C)
        att_out = self.attn(x_att_in, x_att_in, x_att_in)[0]
        att_out = att_out.view(B, nH * nW, ws * ws, C)
        x_win = x_win + att_out
        # MLP
        x_mlp_in = self.norm2(x_win)
        x_win = x_win + self.mlp(x_mlp_in)
        # 还原到图像网格 -> [B, H, W, C]
        x_img = x_win.view(B, nH, nW, ws, ws, C).permute(0, 1, 3, 2, 4, 5).contiguous().view(B, H, W, C)
        # 返回为原始序列形状 [B, N, C]
        return x_img.view(B, N, C)
...
class ClassConditionedUViT(nn.Module):
    def __init__(self, num_classes=len(dataset.classes), class_emb_size=4, vit_dim=256, vit_heads=4, vit_layers=6):
        ...
        # 使用 SwinBlock 替换普通 Transformer blocks
        self.vblocks = nn.ModuleList([SwinBlock(vit_dim, vit_heads, window_size=8) for _ in range(vit_layers)])
    ...
    def forward(self, x, t, class_labels):
        ...
        # SwinBlock 需要当前网格尺寸 gh, gw 以做窗口划分
